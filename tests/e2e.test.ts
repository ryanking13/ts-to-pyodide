...
describe("e2e: Vectorize (.d.ts → IR → renderFile)", () => {
  it("vectorize: declare class, TypedDict options, sub-binding wrapping, array returns", () => {
    const inputDts = readFileSync(
      resolve(FIXTURES_DIR, "vectorize", "input.d.ts"),
      "utf-8",
    );
    const expectedPy = readFileSync(
...
describe("e2e: Queue (.d.ts → IR → renderFile)", () => {
  it("queue: generic interfaces, literal types, TypedDict options, sub-binding wrapping, readonly props", () => {
    const inputDts = readFileSync(
      resolve(FIXTURES_DIR, "queue", "input.d.ts"),
      "utf-8",
    );
    const expectedPy = readFileSync(
      resolve(FIXTURES_DIR, "queue", "expected.py"),
      "utf-8",
    );
...
describe("e2e: options as TypedDict (.d.ts → IR → renderFile)", () => {
  it("kwparams: options bag rendered as TypedDict with snake_case keys", () => {
    const inputDts = readFileSync(
      resolve(FIXTURES_DIR, "kwparams", "input.d.ts"),
      "utf-8",
    );
    const expectedPy = readFileSync(
      resolve(FIXTURES_DIR, "kwparams", "expected.py"),
      "utf-8",
    );
